...
def hh_page():
    ...
    st.markdown(
        """
        The Hodgkin-Huxley model is a mathematical model that describes the generation and propagation
        of action potentials in neurons. The model consists of four ordinary differential equations that
    ...
    sample_idxs = []
    initial_values = [270, 300, 330, 360] if i_max == 4 else [270]
    ...
    if "num_variables_hh" not in st.session_state:
        st.session_state.num_variables_hh = 1

    # Display all the variables
    for var in range(st.session_state.num_variables_hh):
